...
import { company_why_join_us} from "@/scripts/aiprompts";
...
const WhyJoinUsForm = ({ initialData, companyId }: WhyJoinUsFormProps) => {
  ...
  const handlePromptGeneration = async () => {
    try {
      ...
      await getGenerativeAIResponse(company_why_join_us(rollName)).then(
        (data) => {
          data = data.replace(/^'|'$/g, "");
  ...
  return (
    <div className='mt-6 border bg-neutral-100 rounded-md p-4'>
      ...
      {!isEditing && (
        ...
        >
          {!initialData.whyJoinUs && "No Data Provided"}
      ...
      {isEditing && (
        <>
          ...
          <p className='text-xs text-muted-foreground text-right'>
            Note*: Type the Company Name to generate the sample data
          </p>
